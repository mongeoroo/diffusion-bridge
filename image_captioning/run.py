...
def parse_configs():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="whether to use diffmapper",
    )
    ...
    parser.add_argument(
        "--pre_add_gaussian_noise",
        action="store_true",
        default=None,
        help="whether to add gaussian noise to input embeds before diffusion",
    )
    ...
    cfg.data.pre_add_gaussian_noise = args.pre_add_gaussian_noise
    if args.pre_add_gaussian_noise:
        cfg.experiment_name += f"_pre_add_gaussian_noise_level_{round(args.noise_level, 5)}"

    cfg.model.re_normalize_prefix = args.re_normalize_prefix
    if not args.re_normalize_prefix:
        cfg.experiment_name += "_not_renormed"

    cfg.data.use_diffmapper = args.use_diffmapper
    if args.use_diffmapper:
        cfg.experiment_name += "_use_diffmapper"
